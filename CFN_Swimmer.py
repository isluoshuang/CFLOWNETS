...
writer = SummaryWriter(log_dir="runs/CFN_Swimmer_"+now_time)
...
policy.retrieval.load_state_dict(torch.load('retrieval_swimmer_sparse.pkl'))
...
while frame_idx < max_frames:
    ...
    for step in range(max_episode_steps):
        ...
        action_buf.append(action.tolist())
        ...
        if done:
            frame_idx += 1
            print(frame_idx)
            reward_buf[max_episode_steps - 1] = episode_reward
    ...
    if frame_idx % 10 == 0:
        ...
        for i in range(repeat_episode_num):
            ...
            for s in range(max_episode_steps):
                ...
                if test_done:
                    break
            avg_test_episode_reward += math.fabs(test_episode_reward)
        torch.save(policy.network.state_dict(), "runs/gfn_swimmer_" + now_time + '.pkl')
        writer.add_scalar("gfn_swimmer_max_reward", avg_test_episode_reward / repeat_episode_num, global_step=frame_idx * max_episode_steps)
